...
### BUG-005 — Dragging the Now Playing seek bar spams seeks and can visibly fight the user's finger
- Status: VERIFIED
- Severity: Medium
- Confidence: High
- Area: Media3 playback/session, seek/position state, UI correctness
- File/component: `app/src/main/java/com/erdman/erdstream/ui/NowPlayingScreen.kt` (`SliderMMD` usage, ~lines 102-111); `app/src/main/java/com/erdman/erdstream/ErdStreamViewModel.kt` — `seekTo()` (lines 192-195) and the 200ms/1000ms polling loop in `startPlaybackMonitoring()` (lines 238-325).
- Evidence:
  - `com.mudita.mmd.components.slider.SliderMMD` is a Compose-Material-style slider. Confirmed directly from the library's compiled bytecode (`javap` on `mmd-core-release.aar`'s `classes.jar`) that the overload this call site resolves to is `SliderMMD(value: Float, onValueChange: (Float) -> Unit, modifier, enabled, valueRange, steps, onValueChangeFinished: () -> Unit, colors, interactionSource, ...)` — the exact same shape as `androidx.compose.material3.Slider`, where `onValueChange` fires continuously on every drag delta and `onValueChangeFinished` fires once, on release.
  - `NowPlayingScreen.kt`'s call only passes `value` and `onValueChange`; `onValueChangeFinished` is left at its default (a no-op) and is never used to gate anything.
  - `onValueChange`'s lambda calls `onSeek((value * durationMs).toLong()...)` directly on every callback, which is wired straight to `viewModel.seekTo(position, mediaController)` → `controller.seekTo(positionMs)`. This means every pixel of drag movement issues a real `MediaController.seekTo()` call, not just the final release position.
  - Independently, `Column`'s `value = if (durationMs > 0) currentPositionMs.toFloat() / durationMs else 0f` is bound directly to `playbackState.positionMs`, which `ErdStreamViewModel.startPlaybackMonitoring()`'s `while (true)` loop unconditionally overwrites from `controller.currentPosition` every 200ms while playing (1000ms while paused) — with no check for "is a seek/drag currently in progress." A network-streamed seek does not resolve instantly; while it's buffering, `controller.currentPosition` can still reflect the pre-seek (or mid-seek) position for some number of poll ticks, which will overwrite `positionMs` and visibly snap the slider thumb backward mid-drag, away from where the user's finger still is.
  - There is no local "is dragging" state anywhere in `NowPlayingScreen.kt` to decouple the slider's rendered value from the externally-polled position while a drag is in progress, and no use of `onValueChangeFinished` to defer the actual seek to drag-release.
- Reproduction steps:
  1. Play any song long enough to have a duration comfortably over ~30s (a streamed/transcoded track makes the effect more visible, matching this app's real-world usage).
  2. Open Now Playing and slowly drag the seek bar thumb with a finger/pointer, watching closely rather than doing a single quick flick.
  3. Observe: the thumb can jump backward or stutter mid-drag (fighting the finger), and logcat/network inspection shows repeated `stream.view`-adjacent seek activity firing on every drag movement rather than once on release.
- Expected behavior: Dragging should update the slider's visual position locally and smoothly; the actual player seek should happen once, when the drag ends (or at most, be debounced), and the live position poll should not overwrite the slider's displayed position while a drag is actively in progress.
- Current behavior: Every drag pixel triggers a real `MediaController.seekTo()` call, and the concurrent 200ms position poll can overwrite the slider back to the stale pre-seek position while the user is still dragging.
- Proposed regression test: This is a Compose UI interaction bug, not unit-testable against the current JUnit4-only setup; would need a Compose UI test (`createComposeRule`) driving a synthetic drag gesture against `NowPlayingScreen` with a fake position-update ticker, asserting the displayed slider value doesn't regress mid-gesture and that `onSeek` fires only once (or a bounded number of times) per gesture.
- Runtime verification needed: Manual device verification of the repro steps above, pre- and post-fix, ideally against a real transcoded stream (not just a fast local network) so the seek-buffering window is long enough to observe.
- Notes: Found during this round's fresh discovery pass (2026-09-11), independent of BUG-004. Selected as the next task after BUG-004 (see `.ai/CURRENT_TASK.md`) — confirmed via the actual MMD library bytecode rather than assumption, reproducible on every normal seek-bar drag (no special failure condition needed, unlike BUG-004), and directly in `.ai/BUG_HUNT.md`'s explicitly called-out "Media3 playback/session" focus area.
