...
fun SettingsScreen(
    ...
    var backgroundPlaybackExpanded by remember { mutableStateOf(false) }
    var streamingQualityExpanded by remember { mutableStateOf(false) }
    ...
    LazyColumn(
        ...
        item {
            SectionHeader(
                ...
                onToggle = { tabsExpanded = !tabsExpanded },
            )
        }
        if (tabsExpanded) {
            item {
        ...
        item {
            SectionHeader(
                ...
                expanded = backgroundPlaybackExpanded,
                onToggle = { backgroundPlaybackExpanded = !backgroundPlaybackExpanded },
            )
        }
        if (backgroundPlaybackExpanded) {
            item {
                Spacer(modifier = Modifier.height(4.dp))
                Text(
        ...
        item {
            SectionHeader(
                ...
                expanded = streamingQualityExpanded,
                onToggle = { streamingQualityExpanded = !streamingQualityExpanded },
            )
        }
        if (streamingQualityExpanded) {
            item {
                Spacer(modifier = Modifier.height(4.dp))
                Text(
...
/**
 * Tappable section header for a collapsible settings group. Shows a
 * chevron indicating expanded/collapsed state; the section's rows are
 * only composed into the LazyColumn while [expanded] is true, so a
 * collapsed section costs one row of scroll height instead of all its
 * content.
 */
@Composable
